...
if(isset($_POST["txtTitulo"]) && $_POST["txtTitulo"]!="")
{
	...
	if(!empty($_FILES["archivo"]["tmp_name"]))
	{
		...
		$c = conectarBD();
		$qry = "update productos set Titulo='$titulo' where idProducto=$idA";
		mysqli_query($c, $qry);
		$qry = "update productos set NombreOriginal='$nombre' where idProducto=$idA";
		mysqli_query($c, $qry);
		$qry = "update productos set Precio='$precio' where idProducto=$idA";
		mysqli_query($c, $qry);
		$qry = "update productos set Descripcion='$descripcion' where idProducto=$idA";
		mysqli_query($c, $qry);
		$qry = "update productos set TipoContenido='$tipo' where idProducto=$idA";
		mysqli_query($c, $qry);
		$qry = "update productos set Contenido='$contenido' where idProducto=$idA";
		mysqli_query($c, $qry);
		$msg = "<div class='alert alert-success' role='alert'>Se modifico correctamente el producto</div>";
		
		$qry = "delete from listascategorias where idProducto=$idA";
		mysqli_query($c, $qry);
		...
		if($_POST["txtCategoria"])
		{
			...
			foreach($Categoria as $key => $cate)
			{
				...
				$qry = " insert into listascategorias (idCategoria, idProducto) values ('".$categoria->idCategoria."', '$idA')";
				mysqli_query($c, $qry);
			}
		}
	}
...
<form class="container-sm" method="post" enctype="multipart/form-data" action="modificarArchivo.php?idA=<?php echo $_GET['idA']; ?>" onsubmit="return validaFRM()">	
	<a class="btn btn-light float-lg-start" href="administracion.php"><i class="fas fa-arrow-left"></i></a> 
	<div class="container-sm">
		<h3>Formulario para modificar productos</h3>
		<?php
		...
		<div class="d-flex btn-group" role="group">
	  		<input class="form-control me-2 btn btn-outline-primary" type="submit" value="Actualizar Producto">
